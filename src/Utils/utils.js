...
export const validateHash = user => {
  return new Promise((resolve, reject) => {
    axios
      .get(
        `https://ironrest.cyclic.app/findOne/appbooks?usuario=${user.username}`
      )
      .then(response => {
...
export const registerNewUser = data => {
  return new Promise((resolve, reject) => {
    axios
      .get(
        `https://ironrest.cyclic.app/findOne/appbooks?usuario=${data.username}`
      )
      .then(response => {
        if (!response) {
          ...
          sha256(data.password).then(hash => {
            axios
              .post(`https://ironrest.cyclic.app/appbooks`, {
